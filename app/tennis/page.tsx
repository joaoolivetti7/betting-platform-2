...
export default function Bets() {
  ...
  useEffect(() => {
    const fetchBets = async () => {
      try {
        const response = await axios.get(`${API_URL}/tennis/odds`, {
          params: {
            apiKey: API_KEY,
            regions: "us",
            oddsFormat: "decimal",
  ...
  return (
    <div className=" white py-8 flex items-center justify-center">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-bold text-white mb-6">Apostas de Tênis</h1>
                  ...
                  <CardContent>
                    ...
                    {bet.bookmakers[0]?.markets.map((market) => (
                      <div key={market.key} className="mb-2">
                        {/* <p className="text-sm text-gray-600">{market.key === 'h2h' ? 'Moneyline' : 'Spread'}</p> */}
